...
class ModelRegistry:
    ...
    def _try_load(self, name: str, path: Path, use_dill: bool = False):
        try:
            if not path.exists():
                # Fallback to retail_clv model path if it exists
                fallback_path = PROJECT_ROOT.parent / "retail_clv" / "models" / path.name
                if fallback_path.exists():
                    log.info("Model not in platform directory. Falling back to retail_clv model: %s", fallback_path.name)
                    path = fallback_path
                else:
                    log.warning("Model file not found: %s", path.name)
...
def predict(customer_id: str, transactions: list[dict]) -> dict[str, Any]:
    ...
    if _registry.has_bg_nbd and features["frequency"] > 0:
        ...
        messages.append("Only 1 transaction available — using simple revenue projection.")
    ...
    if clv_bg_nbd > 0 and clv_xgb > 0:
        ...
        clv_final = features["monetary_value"] * 2
    ...
    return {
        ...
        "churn_risk_tier": get_churn_tier(churn_risk),
